...
| `ChatBridge.dc.html` | 閒聊 → 心跡 → 起卦：擬題確認卡的三種狀態（**墨底**，見下） |
...
`ChatBridge.dc.html` 是唯一一張墨底的——它畫的卡片出現在**閒聊畫面**上，
而閒聊是墨底的。畫成宣紙色只會讓人以為心跡已經換皮了。這張的存在正好把
下面那個還沒下的決定攤開來：兩套配色在同一條動線上會前後撞見。
