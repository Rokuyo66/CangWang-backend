...
-- 【⚠ 下面那些價格是佔位值，不是建議售價】
--
-- 定價要看成本。用 0043 的單價表推算「該階被用好用滿」的每月成本（假設快取
-- 命中、不含深論與換人評卦、匯率 32）：
--
--   方案      AI／月   TTS／月    合計
--   ─────────────────────────────────
...
-- 三件事值得先想：
--
-- 一、藏往用滿要 864。訂價低於它就是「越重度的用戶虧越多」，而重度用戶正是
--     最不會退訂的那群——虧損會穩定成長，看起來卻像留存很好。
-- 二、TTS 在高階佔比極高（藏往有 192 是純朗讀）。真要壓成本，換 turbo 模型
--     可以省四成，那是一個環境變數，不必改程式（見 dev/plan-gating-gaps.md）。
-- 三、免費帳號用滿也要 102，而那一側完全沒有收入。目前唯一的煞車是
--     DAILY_GLOBAL_CAP（現值 200 次／日全站）。
--
-- 但「用好用滿」是天花板，不是平均。真正該照著定價的數字是 ai_cost() 的
-- by_plan.twd_per_user_per_day 與 top_users——那要等真實流量才有。
-- 所以下面填的是「等於成本天花板」的保守值：不會虧，但幾乎肯定訂得太高。
-- 開賣前照你的判斷 update 一次，不必改程式也不必出 migration。
...
-- 佔位值，見檔頭。lingshi_grant 取「約當該階每日免費追問用完後、還能再追問幾次」的量級。
insert into plans (id, label, twd, lingshi_grant, blurb, sort) values
  ('guanwei',  '觀微', 183,  60, '每日三卦三追問，記憶加深一層',     1),
  ('zhiji',    '知幾', 419, 150, '每日五卦八追問，心跡同時記八件事', 2),
  ('cangwang', '藏往', 864, 300, '每日八卦二十追問，朗讀六萬字',     3)
