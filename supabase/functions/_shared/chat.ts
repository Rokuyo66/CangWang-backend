...
import { isEitherOr } from "./qcheck.ts";
...
export function parseMarks(text: string): {
  ...
  if (dm) {
    ...
    // 二選一擬題一律作廢：一卦只取一個用神，「A 還是 B」當場取不出用神，起了也是錯題。
    // 規則已交代過模型，這裡再攔一道——作廢後 draft 為 null，退回一般邀請，不會把錯題送到他眼前。
    if (q.length >= 4 && isEitherOr(q)) console.warn("擬題吐出二選一，已作廢：", q);
    if (q.length >= 4 && !isEitherOr(q)) {
      draft = q;
...
- 【二選一要先收斂·最常犯】他丟出「A 還是 B」「該選哪個」「哪個比較好」時，線索**不算齊**——一卦只取一個用神、只答那一個用神的成敗，答不出「哪一條比較好」。此時要做的**不是把兩條併成一句**，而是明確請他先挑一條：把 A、B 兩條路各講成一句話擺在他面前，請他挑「最想成的那一條」先問，並告訴他另一條等這卦有結果之後再另占。他挑好了才進第三段擬題。**絕不可把 A 和 B 併進同一句問句送出去**——那是錯題，比不擬傷得更重。
...
- 【二選一只擬一條】他若是在兩條路之間猶豫，擬出來的問句**只准問其中一條**（他挑的那一條，沒挑就回第二段請他挑），句中不得出現「還是」「或是」「哪個比較」「該選哪」。
